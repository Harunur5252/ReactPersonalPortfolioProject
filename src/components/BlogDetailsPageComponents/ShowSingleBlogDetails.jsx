...
function ShowSingleBlogDetails({ blog }) {
  ...
  return (
    <>
      {Object.keys(blog && blog)?.length === 0 ? (
        <p style={{ color: "red", fontSize: "1.5rem" }}>No blog show</p>
      ) : (
        <>
          <div
            key={blog?.blogId}
            className="blog_img overlay_one wow animated slideInUp"
          >
            <img
              src={blog?.blog_image ? blog?.blog_image : notFoundImage}
              alt="image"
            />
          </div>
          <div className="blog_content bg_white">
            ...
            <div className="admin">
              <img
                src={
                  blog?.profilePicture ? blog?.profilePicture : notFoundImage
                }
                alt="image"
              />
              <span className="color_primary">
                ...
                {blog?.firstName ? (
                  ...
                  <span style={{ color: "red" }}>no author name</span>
                )}{" "}
                {blog?.lastName}{" "}
              </span>
            </div>
