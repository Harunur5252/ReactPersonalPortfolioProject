...
const schema = yup.object({
  password: yup
    ...
    .matches(
      /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{6,})/,
      "Must Contain 6 character,One Uppercase,One Lowercase,One Number and One special case character"
    ),
...
function ResetPassword() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setSubmit(false);
      toast.success(
        "password resets successfully,please login with update password"
      );
      navigate("/login");
    } catch (err) {
      setSubmit(false);
