...
function Comment({ comment, blogId }) {
  ...
  const repliedComments = comment?.replay_comments?.data?.filter((replay) => {
    if (
      replay?.attributes?.comment?.data?.attributes?.blog_post?.data?.id ===
        comment?.blogId &&
      replay?.attributes?.comment?.data?.id === comment?.cmtId &&
      (replay?.attributes?.user?.data?.id === user?.id ||
        replay?.attributes?.user?.data?.id !== user?.id)
    ) {
      return replay;
    }
            ...
            <div className="author_info">
              <h5 className="author_name color_primary">
                {comment?.firstName ? (
                  ...
                  <span style={{ color: "red" }}>no firstName</span>
                )}{" "}
                {comment?.lastName ? (
                  comment?.lastName
                ) : (
              ...
              <span>
                {comment?.commentDate &&
                format(new Date(comment?.commentDate), "dd MMMM, yyyy p") ? (
                  comment?.commentDate &&
                  format(new Date(comment?.commentDate), "dd MMMM, yyyy p")
                ) : (
                  <span style={{ color: "red" }}>no published date</span>
                )}
              </span>
            </div>
            <p>
          ...
          <li key={repliedComment?.id} className="mb_20 replied">
            <div className="comment_description bg_white p_20">
              <div className="author_img">
                <img
                  src={
                    repliedComment?.attributes?.user?.data?.attributes?.profile
                      ?.data?.attributes?.profilePicture?.data?.attributes?.url
                      ? repliedComment?.attributes?.user?.data?.attributes
                          ?.profile?.data?.attributes?.profilePicture?.data
                          ?.attributes?.url
                      : notFoundImage
                  }
                  alt="images"
              ...
              <div className="author_text">
                <div className="author_info">
                  ...
                  <span>
                    {repliedComment?.attributes?.replayDate &&
                    format(
                      new Date(repliedComment?.attributes?.replayDate),
                      "dd MMMM, yyyy p"
                    ) ? (
                      repliedComment?.attributes?.replayDate &&
                      format(
                ...
                <div>
                  <p>
                    {repliedComment?.attributes?.description ? (
                      repliedComment?.attributes?.description
                    ) : (
                      <span style={{ color: "red" }}>no description</span>
                    )}
              ...
              <div className="row">
                <div className="col-md-12">
                  <textarea
                    ...
                    rows="3"
                    placeholder="Type replay comment..."
                  ></textarea>
