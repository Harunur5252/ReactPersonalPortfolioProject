...
export function PageProvider({ children }) {
  ...
  const [loadedTestimonialSection, setLoadedTestimonialSection] =
    useState(false);
  ...
  const handleColorSet = async (e, color) => {
    e.preventDefault();
    try {
  ...
  const loadColor = async () => {
    try {
      const res = await axiosPublicInstance.get("/colors");
      for (const colorItem of res?.data?.data) {
        setColorData({
          id: colorItem?.id,
          colorName: colorItem?.attributes?.colorName,
        });
      }
    } catch (err) {
  ...
  const loadHeroSection = async () => {
    try {
      ...
      const heroArr = response.data?.data?.attributes?.homeFeatured?.map(
        (hero) => {
          return {
  ...
  const loadPortfolioSection = async () => {
    ...
    try {
      ...
      const tagsArr = response.data?.data?.attributes?.ProjectsFeature?.map(
        (data) => {
          return {
            id: data?.id,
            image: data?.image,
  ...
  const loadTestimonialSection = async () => {
    const query = qs.stringify({
      populate: ["testimonialFeature", "testimonialFeature.FeatureFeedback"],
    });
    try {
