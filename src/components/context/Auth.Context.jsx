...
export function AuthProvider({ children }) {
  ...
  const passwordChange = async (data) => {
    try {
      ...
      toast.success(
        "Password change successfully,now you can login with new password",
        {
          position: "top-right",
          autoClose: 2000,
  ...
  const userBlogUpdate = async (data, foundBlog, des) => {
    ...
    try {
      if (
        ...
        (blogData.title || blogData.description || blogData.blog_date) &&
        data?.blog_image[0] &&
        !foundBlog?.imgId
      ) {
        const formData = new FormData();
    ...
    try {
      ...
      const profileArr = response.data.data?.map((profile) => {
        return {
          ...
          profilePicture: profile?.attributes?.profilePicture?.data?.attributes
            ?.formats?.thumbnail?.url
            ? profile?.attributes?.profilePicture?.data?.attributes?.formats
                ?.thumbnail?.url
            : profile?.attributes?.profilePicture?.data?.attributes?.url,
  ...
  const login = async (data) => {
    try {
      ...
      dispatch({
        type: LOGIN,
        payload: { data: response.data, setUser, setToken },
      });
